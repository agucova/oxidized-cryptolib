...
impl BenchmarkPrinter {
    ...
    pub fn print_summary(&self, all_stats: &[BenchmarkStats]) {
        ...
        for (benchmark_name, stats) in by_benchmark.iter() {
            if stats.len() < 2 {
                continue;
            }
            ...
            if self.color {
                ...
                println!("  {} ran", fastest_name);
            }

            // Compare to other implementations
            let mut comparisons: Vec<_> = stats
                .iter()
                ...
                .map(|other| {
                    ...
                    // Estimate uncertainty from std devs (simplified propagation)
                    let rel_err_fastest = fastest.latency.std_dev.as_nanos() as f64
                        / fastest.latency.mean.as_nanos() as f64;
                    let rel_err_other = other.latency.std_dev.as_nanos() as f64
                        / other.latency.mean.as_nanos() as f64;
                    let uncertainty = speedup * (rel_err_fastest.powi(2) + rel_err_other.powi(2)).sqrt();
                    (other, speedup, uncertainty)
                })
                .collect();

            // Sort by speedup (fastest comparisons first)
            comparisons.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
            ...
            for (other, speedup, uncertainty) in comparisons {
                ...
                if self.color {
                    ...
                    println!("    {} times faster than {}", speedup_str, other_name);
                }
            }
        }
        println!();
            ...
            for other in stats.iter().filter(|s| s.implementation != fastest.implementation) {
                ...
                if let Some(comp) = comparison {
                    ...
                    if self.color {
                        ...
                        println!(
                            "    {} times faster than {}{}",
                            speedup_str, other_name, significance
                        );
                    }
                } else {
...
mod tests {
    ...
    fn make_stats(name: &str, impl_type: Implementation, mean_ms: u64) -> BenchmarkStats {
        BenchmarkStats {
            ...
            file_size: Some(FileSize::Small4KB),
            sample_count: 50,
    ...
    fn test_print_result_no_panic() {
        ...
        let stats = make_stats("read_4KB", Implementation::OxidizedFuse, 5);
    ...
    fn test_print_summary_single_impl() {
        let printer = BenchmarkPrinter::new(false);
        let stats = vec![make_stats("read_4KB", Implementation::OxidizedFuse, 5)];
    ...
    fn test_print_summary_multiple_impls() {
        let printer = BenchmarkPrinter::new(false);
        let stats = vec![
            make_stats("read_4KB", Implementation::OxidizedFuse, 5),
            make_stats("read_4KB", Implementation::OxidizedWebDav, 10),
        ];
        printer.print_summary(&stats);
    }
}
