...
use oxcrypt_mount::{get_system_mounts_detailed, is_our_mount};
...
/// oxcrypt-core (via `is_our_mount()`) before attempting cleanup.
    ...
    if path.exists() && is_mount_point(path).unwrap_or(false) {
        ...
        if let Some(mount) = mount_entry {
            if is_our_mount(mount) {
                tracing::warn!(
                    "Detected stale oxcrypt-core mount at {} (fsname: {}), cleaning up...",
                    path.display(),
                    mount.fsname
                );
