...
// Photo sizes for synthetic generation
...
const NUM_SMALL_PHOTOS: usize = 50;
const NUM_MEDIUM_PHOTOS: usize = 20;
const NUM_LARGE_PHOTOS: usize = 10;
...
pub struct PhotoLibraryWorkload {
    seed: u64,
    /// Whether to use real downloaded photo assets.
    use_real_assets: bool,
}

impl PhotoLibraryWorkload {
    /// Create a new photo library workload with synthetic content.
    pub fn new() -> Self {
        Self {
            seed: 0x0F07_01AB,
    ...
    /// Create a new photo library workload with real downloaded assets.
    ...
    fn workload_dir(&self, mount_point: &Path) -> PathBuf {
        mount_point.join("bench_photo_workload")
    }

    fn photos_dir(&self, mount_point: &Path) -> PathBuf {
        self.workload_dir(mount_point).join("photos")
    }

    fn export_dir(&self, mount_point: &Path) -> PathBuf {
        self.workload_dir(mount_point).join("export")
    }
    ...
    fn list_photos(&self, mount_point: &Path) -> Result<Vec<PathBuf>> {
        let photos_dir = self.photos_dir(mount_point);
...
impl Benchmark for PhotoLibraryWorkload {
    ...
    fn parameters(&self) -> HashMap<String, String> {
        ...
        if self.use_real_assets {
            ...
            let total_photos = NUM_SMALL_PHOTOS + NUM_MEDIUM_PHOTOS + NUM_LARGE_PHOTOS;
            let total_size = NUM_SMALL_PHOTOS * SMALL_PHOTO_SIZE
                + NUM_MEDIUM_PHOTOS * MEDIUM_PHOTO_SIZE
                + NUM_LARGE_PHOTOS * LARGE_PHOTO_SIZE;
    ...
    fn setup(&self, mount_point: &Path) -> Result<()> {
        // Create directories
        fs::create_dir_all(self.photos_dir(mount_point))?;
        fs::create_dir_all(self.export_dir(mount_point))?;
                ...
                Ok(asset_paths) => {
                    ...
                    for (i, src_path) in asset_paths.iter().enumerate() {
                        let filename = src_path.file_name()
                            ...
                            .unwrap_or_else(|| format!("photo_{:04}.jpg", i));
        ...
        for _ in 0..NUM_SMALL_PHOTOS {
            let content = self.generate_synthetic_photo(&mut rng, SMALL_PHOTO_SIZE);
            let path = photos_dir.join(format!("photo_{:04}.jpg", photo_num));
        ...
        for _ in 0..NUM_LARGE_PHOTOS {
            let content = self.generate_synthetic_photo(&mut rng, LARGE_PHOTO_SIZE);
            let path = photos_dir.join(format!("photo_{:04}.raw", photo_num));
            let mut file = File::create(&path)?;
            file.write_all(&content)?;
            safe_sync(&file)?;
            photo_num += 1;
    ...
    fn run(&self, mount_point: &Path) -> Result<Duration> {
        ...
        let photos = self.list_photos(mount_point)?;
        if photos.is_empty() {
        ...
        {
            let sample_count = 10.min(photos.len());
        ...
        {
            let slideshow_count = 20.min(photos.len());
        ...
        {
            let export_dir = self.export_dir(mount_point);
            let export_count = 5.min(photos.len());
            let mut indices: Vec<usize> = (0..photos.len()).collect();
            indices.shuffle(&mut rng);
