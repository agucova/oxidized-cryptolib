        ...
        group.bench_with_input(BenchmarkId::from_parameter(name), &size, |b, &size| {
            ...
            b.iter(|| {
                ...
                let decrypted_content =
                    decrypt_file_content(&ciphertext, &decrypted_header.content_key, &header_nonce)
                        .unwrap();
    ...
    for (name, size) in test_sizes {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(name), &size, |b, &size| {
            ...
            b.iter(|| {
                ...
                let ciphertext =
                    encrypt_file_content(&plaintext, &content_key, &header_nonce).unwrap();
                let encrypted_header = encrypt_file_header(&content_key, &master_key).unwrap();
...
fn bench_chunked_operations(c: &mut Criterion) {
    ...
    for (name, size) in test_sizes {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(name), &size, |b, &size| {
            ...
            b.iter(|| {
                let decrypted =
                    decrypt_file_content(&ciphertext, &content_key, &header_nonce).unwrap();
                black_box(decrypted);
            });
        });
    }
    ...
    group.bench_function("streaming_1MB_reads", |b| {
        b.iter(|| {
            ...
            if end_offset > start_offset {
                ...
                let decrypted =
                    decrypt_file_content(partial_ciphertext, &content_key, &header_nonce)
...
fn bench_symlink_operations(c: &mut Criterion) {
    ...
    let test_targets = [
        ...
        (
            "medium_relative",
            "../some/nested/directory/structure/file.txt",
        ),
        (
            "long_absolute",
            &format!("/home/user/documents/{}/file.txt", "a".repeat(200)),
        ),
        (
            "very_long",
            &format!("/path/to/{}/deeply/nested/target.txt", "x".repeat(500)),
        ),
    ];
    ...
    for (name, target) in &test_targets {
        group.throughput(Throughput::Bytes(target.len() as u64));
        group.bench_with_input(BenchmarkId::new("encrypt", *name), target, |b, target| {
            b.iter(|| {
                let encrypted = encrypt_symlink_target(target, &master_key).unwrap();
                black_box(encrypted);
            });
