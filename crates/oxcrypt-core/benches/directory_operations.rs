            ...
            b.iter(|| {
                ...
                if path.is_empty() {
                    ...
                    for _component in components {
                        current_id = hash_dir_id(&current_id, &master_key).unwrap();
                    }
    ...
    group.bench_function("bulk_rename_simulation", |b| {
        b.iter(|| {
            for (old_name, new_name) in &file_pairs {
                ...
                let encrypted_new = encrypt_filename(new_name, dir_id, &master_key).unwrap();

                // In a real scenario, we'd also decrypt the old name to verify
                let encrypted_old = encrypt_filename(old_name, dir_id, &master_key).unwrap();
                let decrypted_verify = decrypt_filename(&encrypted_old, dir_id, &master_key);
