mod crypto_tests {
    ...
    fn test_header_with_non_standard_reserved_bytes() {
        ...
        let encrypted_header = master_key
            ...
            .expect("key access should succeed")
            .expect("encryption should succeed");
    ...
    fn test_header_encryption_uses_standard_reserved_bytes() {
        ...
        let encrypted_header =
            encrypt_file_header(&content_key, &master_key).expect("encryption should succeed");
        ...
        let decrypted_plaintext = master_key
            .with_aes_key(|aes_key| {
                let key: &Key<Aes256Gcm> = aes_key.into();
                let cipher = Aes256Gcm::new(key);
    ...
    fn test_header_various_reserved_byte_patterns() {
        ...
        for reserved in &reserved_patterns {
            ...
            assert_eq!(
                *decrypted.content_key, content_key,
...
mod ctrmac_tests {
    use oxcrypt_core::crypto::keys::MasterKey;
    use oxcrypt_core::error::FileContext;
    use oxcrypt_core::fs::file_ctrmac::{
        NONCE_SIZE, PAYLOAD_SIZE, decrypt_content, decrypt_header, encrypt_content, encrypt_header,
    };
    use proptest::prelude::*;
    use rand::RngCore;
...
fn test_decrypt_java_dirid_c9r_files() {
    ...
    use oxcrypt_core::fs::file::{decrypt_file_content, decrypt_file_header};
    ...
    if root_dirid_path.exists() {
        ...
        assert_eq!(
            decrypted_str, "",
            "Root's dirid.c9r should contain empty string (root's own ID)"
        );
    }
    ...
    if subfolder_dirid_path.exists() {
        let encrypted_data =
            fs::read(&subfolder_dirid_path).expect("Failed to read subfolder dirid.c9r");
        println!(
            "Subfolder dirid.c9r file size: {} bytes",
            encrypted_data.len()
        );
        ...
        let decrypted_str = String::from_utf8(content).expect("content should be valid UTF-8");
        println!(
            "Subfolder dirid.c9r content: '{}' (length: {})",
            decrypted_str,
            decrypted_str.len()
        );
        ...
        assert_eq!(
            decrypted_str, "e9250eb8-078d-4fc0-8835-be92a313360c",
