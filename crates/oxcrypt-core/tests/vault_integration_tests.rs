...
use common::{
    ...
    test_data::{
        patterns,
        sizes::{
            CHUNK_MINUS_ONE, CHUNK_PLUS_ONE, CHUNK_SIZE, LARGE, TWO_CHUNKS, TWO_CHUNKS_PLUS_ONE,
        },
    },
...
fn test_empty_file_handling() {
    let (vault_path, master_key) =
        create_test_vault_with_files(vec![("empty.txt", b""), ("dir/empty.dat", b"")]);
...
fn test_special_characters_in_content() {
    ...
    let decrypted = vault_ops
        .read_file(&DirId::root(), "special_text.txt")
        .unwrap();
    ...
    // Test null bytes
    let decrypted = vault_ops
        .read_file(&DirId::root(), "null_bytes.dat")
        .unwrap();
...
fn test_chunk_boundary_files() {
    let test_cases = vec![
        (
            "chunk_minus_one.bin",
            test_files::create_sized_content(CHUNK_MINUS_ONE),
        ),
        (
            "chunk_exact.bin",
            test_files::create_sized_content(CHUNK_SIZE),
        ),
        (
            "chunk_plus_one.bin",
            test_files::create_sized_content(CHUNK_PLUS_ONE),
        ),
        (
            "two_chunks.bin",
            test_files::create_sized_content(TWO_CHUNKS),
        ),
        (
            "two_chunks_plus.bin",
            test_files::create_sized_content(TWO_CHUNKS_PLUS_ONE),
        ),
    ];
...
fn test_very_long_filenames() {
    ...
    let decrypted = vault_ops
        .read_file(&DirId::root(), &very_long_name)
        .unwrap();
...
fn test_large_files() {
    ...
    let (vault_path, master_key) =
        create_test_vault_with_files(vec![("large.bin", &large_content)]);
...
fn test_edge_case_vault() {
    ...
    let chunk_file = vault_ops
        .read_file(&DirId::root(), "chunk_boundary.bin")
        .unwrap();
    assert_eq!(
        chunk_file.content,
        test_files::create_chunk_boundary_content()
    );

    // Test special characters
    let special = vault_ops
        .read_file(&DirId::root(), "special_chars.txt")
        .unwrap();
    assert_eq!(special.content, test_files::create_special_char_content());

    // Test deeply nested file
    let (dir_id, _) = vault_ops
        .resolve_path("nested/deeply/nested/structure")
        .unwrap();
...
fn test_entry_type_root() {
    ...
    let (vault_path, master_key) = create_test_vault_with_files(vec![("file.txt", b"content")]);
    let vault_ops = VaultOperations::new(&vault_path, master_key);
...
fn test_entry_type_file() {
    ...
    assert_eq!(
        vault_ops.entry_type("docs/readme.md"),
        Some(EntryType::File)
    );
}
...
fn test_entry_type_directory() {
    ...
    // Nested directory
    assert_eq!(
        vault_ops.entry_type("docs/nested"),
...
fn test_entry_type_symlink() {
    ...
    // Create a symlink
    vault_ops
        .create_symlink(&DirId::root(), "link.txt", "file.txt")
        .unwrap();
...
fn test_list_unified_entries() {
    ...
    // Create a symlink
    vault_ops
        .create_symlink(&DirId::root(), "link.txt", "file1.txt")
        .unwrap();
...
fn test_list_by_path() {
    ...
    let file_names: Vec<_> = docs_entries
        .iter()
        .filter(|e| e.is_file())
        .map(oxcrypt_core::vault::DirEntry::name)
        .collect();
    let dir_names: Vec<_> = docs_entries
        .iter()
...
fn test_create_directory_all() {
    ...
    assert_eq!(
        vault_ops.entry_type("a"),
        Some(oxcrypt_core::vault::path::EntryType::Directory)
    );
    assert_eq!(
        vault_ops.entry_type("a/b"),
        Some(oxcrypt_core::vault::path::EntryType::Directory)
    );
    assert_eq!(
        vault_ops.entry_type("a/b/c"),
        Some(oxcrypt_core::vault::path::EntryType::Directory)
    );
    assert_eq!(
        vault_ops.entry_type("a/b/c/d"),
        Some(oxcrypt_core::vault::path::EntryType::Directory)
    );
...
fn test_touch() {
    ...
    assert_eq!(
        vault_ops.entry_type("newfile.txt"),
        Some(oxcrypt_core::vault::path::EntryType::File)
    );
...
#[test]
fn test_append() {
    let (vault_path, master_key) = VaultBuilder::new().add_file("log.txt", b"Line 1\n").build();
    let vault_ops = VaultOperations::new(&vault_path, master_key);
...
fn test_rename_directory_by_path() {
    ...
    // Rename directory
    vault_ops
        .rename_directory_by_path("projects/old_name", "new_name")
        .unwrap();
    ...
    let content = vault_ops
        .read_by_path("projects/new_name/readme.txt")
        .unwrap();
    assert_eq!(&content.content, b"content");
}
...
fn test_symlink_by_path_operations() {
    ...
    vault_ops
        .create_symlink_by_path("links/mylink", "../target.txt")
        .unwrap();
...
fn test_get_entry_returns_directory() {
    let (vault_path, master_key) = VaultBuilder::new().add_directory("projects/web").build();
    let vault_ops = VaultOperations::new(&vault_path, master_key);
...
fn test_get_entry_returns_symlink() {
    let (vault_path, master_key) = VaultBuilder::new().add_directory("links").build();
    let vault_ops = VaultOperations::new(&vault_path, master_key);

    // Create symlink
    vault_ops
        .create_symlink_by_path("links/mylink", "/etc/hosts")
        .unwrap();
