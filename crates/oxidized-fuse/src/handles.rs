...
//! using the shared [`WriteBuffer`] from `oxidized-mount-common` for
//! random-access writes, and [`HandleTable`] for thread-safe handle management.

// Re-export shared types for convenience
pub use oxidized_mount_common::{HandleTable, WriteBuffer};
...
pub enum FuseHandle {
    ...
    Reader(Box<oxidized_cryptolib::fs::streaming::VaultFileReader>),
...
impl FuseHandle {
    ...
    pub fn as_reader_mut(
        &mut self,
    ) -> Option<&mut oxidized_cryptolib::fs::streaming::VaultFileReader> {
        match self {
...
mod tests {
    ...
    // Note: WriteBuffer tests are in oxidized-mount-common.
