...
//!
//! Run: `cargo nextest run -p oxcrypt-fuse --features fuse-tests move_copy_tests`
...
// =============================================================================
// RENAME_EXCHANGE (Atomic Swap) - Linux Only
// =============================================================================

/// Test atomic exchange of two files in the same directory.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_same_dir() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());
    ...
    // Atomic swap
    mount
        .rename_exchange("file_a.txt", "file_b.txt")
...
/// Test atomic exchange of two files in different directories.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_cross_dir() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());
    ...
    mount
        .write("dir_a/file.txt", b"content A")
        .expect("write a failed");
    mount
        .write("dir_b/file.txt", b"content B")
        .expect("write b failed");

    // Atomic swap across directories
    mount
        .rename_exchange("dir_a/file.txt", "dir_b/file.txt")
        .expect("rename_exchange failed");

    // Contents should be swapped
    assert_file_content(&mount, "dir_a/file.txt", b"content B");
    assert_file_content(&mount, "dir_b/file.txt", b"content A");
}

/// Test atomic exchange of two directories in the same parent.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_directories() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());
    ...
    mount
        .write("dir_a/file.txt", b"in A")
        .expect("write in a failed");
    mount
        .write("dir_b/file.txt", b"in B")
        .expect("write in b failed");

    // Atomic swap of directories
    mount
        .rename_exchange("dir_a", "dir_b")
        .expect("rename_exchange failed");

    // Directory contents should be swapped
    assert_file_content(&mount, "dir_a/file.txt", b"in B");
    assert_file_content(&mount, "dir_b/file.txt", b"in A");
}

/// Test that exchanging a file and directory fails with EINVAL.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_type_mismatch_fails() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());
    ...
    // Swapping file with directory should fail
    let result = mount.rename_exchange("file.txt", "dir");
    assert!(result.is_err(), "exchange of file and dir should fail");

    // Check errno is EINVAL
    let err = result.unwrap_err();
    assert_eq!(
        err.raw_os_error(),
        Some(libc::EINVAL),
        "expected EINVAL for type mismatch"
    );
}

/// Test that exchanging directories across different parents fails with EXDEV.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_cross_dir_dirs_fails() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());
    ...
    assert_eq!(
        err.raw_os_error(),
        Some(libc::EXDEV),
        "expected EXDEV for cross-parent dir swap"
    );
}

/// Test that exchange with non-existent source fails with ENOENT.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_source_not_found() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());

    mount.write("exists.txt", b"content").expect("write failed");

    let result = mount.rename_exchange("not_exists.txt", "exists.txt");
    assert!(result.is_err(), "exchange with missing source should fail");

    let err = result.unwrap_err();
    assert_eq!(
        err.raw_os_error(),
        Some(libc::ENOENT),
        "expected ENOENT for missing source"
    );
}

/// Test that exchange with non-existent target fails with ENOENT.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_target_not_found() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());

    mount.write("exists.txt", b"content").expect("write failed");

    let result = mount.rename_exchange("exists.txt", "not_exists.txt");
    assert!(result.is_err(), "exchange with missing target should fail");

    let err = result.unwrap_err();
    assert_eq!(
        err.raw_os_error(),
        Some(libc::ENOENT),
        "expected ENOENT for missing target"
    );
}

/// Test atomic exchange preserves file content integrity for large files.
#[test]
#[cfg(target_os = "linux")]
fn test_rename_exchange_large_files() {
    skip_if_no_fuse!();
    let mount = require_mount!(TestMount::with_temp_vault());

    let content_a = multi_chunk_content(3);
    let content_b = multi_chunk_content(5);
    ...
    // Atomic swap
    mount
        .rename_exchange("large_a.bin", "large_b.bin")
        .expect("rename_exchange failed");

    // Verify content integrity after swap
    assert_file_hash(&mount, "large_a.bin", &hash_b);
    assert_file_hash(&mount, "large_b.bin", &hash_a);
}
