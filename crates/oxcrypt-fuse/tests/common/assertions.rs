//! FUSE-specific assertions for integration tests.
//!
//! Provides assertions that work with `TestMount` and verify filesystem
//! behavior through the mounted FUSE interface.

// Not all tests use all assertions
#![allow(dead_code)]

use super::harness::TestMount;
use oxcrypt_mount::testing::{assert_bytes_equal, sha256};
...
pub fn assert_errno<T: std::fmt::Debug>(
    ...
    oxcrypt_mount::testing::assertions::assert_errno(result, expected_errno, context);
}
