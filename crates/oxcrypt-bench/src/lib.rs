...
//! - **External**: Any externally-mounted vault (e.g., official Cryptomator app, Mountain Duck)
//!
//! # Usage
//!
//! ```text
...
//!   [implementations]... Implementations to benchmark (fuse, fskit, webdav, nfs, external)
//!
//! Options:
//!   -m, --mount-prefix <PATH>      Mount point prefix (default: /tmp/oxbench)
//!   -e, --external-vault <PATH>    Path to already-mounted external vault
//!   -p, --password <PASSWORD>      Vault password (or OXBENCH_PASSWORD env)
//!   -s, --suite <SUITE>            Benchmark suite: quick, read, write, synthetic, workload, complete
