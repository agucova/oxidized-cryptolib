...
use oxcrypt_core::vault::config::{extract_master_key, validate_vault_claims};
...
#[cfg(any(feature = "fuse", feature = "fskit", feature = "webdav", feature = "nfs"))]
...
fn resolve_vault_path(path: &PathBuf) -> Result<PathBuf> {
    ...
    if io::stdin().is_terminal() {
        anyhow::bail!(
            "--password-stdin requires password to be piped in.\n\
             Example: echo \"$SECRET\" | oxcrypt vault --password-stdin ls /"
        );
    }
...
fn read_password_from_fd(fd: i32) -> Result<String> {
    ...
    if password.is_empty() {
        anyhow::bail!("Password from file descriptor {} is empty", fd);
    }
...
fn unlock_vault(vault_path: &PathBuf, password_opts: &PasswordOptions) -> Result<VaultOperations> {
    let passphrase = get_passphrase(password_opts)?;
...
/// Categorize an error into an exit code
fn categorize_error(e: &anyhow::Error) -> u8 {
    let msg = format!("{e:#}").to_lowercase();

    if msg.contains("passphrase") || msg.contains("password") || msg.contains("master key") {
    ...
    } else if msg.contains("mount") || msg.contains("unmount") {
