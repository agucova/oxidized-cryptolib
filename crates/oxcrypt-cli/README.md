...
```bash
cargo install --path crates/oxcrypt-cli
```
...
#### `mount` - Mount a vault as a filesystem

```bash
oxcrypt mount /path/to/vault /mnt/vault           # Auto-select best backend
oxcrypt mount /path/to/vault /mnt/vault -b fuse   # Use FUSE backend
oxcrypt mount /path/to/vault /mnt/vault -b webdav # Use WebDAV backend
```

#### `unmount` - Unmount a mounted vault

```bash
oxcrypt unmount /mnt/vault
```
...
#### `backends` - List available mount backends
...
# Mount and access vault
oxcrypt mount ~/Vaults/personal /mnt/vault
ls /mnt/vault
oxcrypt unmount /mnt/vault
```
...
cargo bench -p oxcrypt-cli

# Quick benchmark run (faster, less precise)
cargo bench -p oxcrypt-cli -- --quick
```
