...
public class XPCPasswordClient {
    ...
    public func getPassword(domainId: String) -> String {
        guard let connection = connection else {
            return "ERROR:-1:Not connected to XPC service"
        }
        ...
        guard let service = proxy as? PasswordRetrievalProtocol else {
            return "ERROR:-2:Invalid proxy type"
        }

        service.getPassword(forDomain: domainId) { password, error in
            if let error = error {
        ...
        if semaphore.wait(timeout: timeout) == .timedOut {
            ...
            return "ERROR:-4:XPC request timed out"
        }
