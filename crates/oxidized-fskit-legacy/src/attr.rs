...
use oxidized_mount_common::moka_cache::{CachedEntry, SyncTtlCache, DEFAULT_TTL};
use oxidized_mount_common::CacheStats;
use std::sync::Arc;
use std::time::Duration;

// Re-export for backwards compatibility
pub use oxidized_mount_common::moka_cache::DEFAULT_TTL as DEFAULT_ATTR_TTL;
...
/// by caching recently accessed file attributes. Uses [`SyncTtlCache`] internally.
pub struct AttrCache {
    /// The underlying cache.
    cache: SyncTtlCache<u64, ItemAttributes>,
}
...
mod tests {
    ...
    fn test_cache_clear() {
        ...
        cache.clear();
        cache.cleanup_expired(); // Force pending clear
