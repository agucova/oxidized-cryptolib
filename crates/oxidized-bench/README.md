...
- **FUSE** - Our FUSE implementation (`oxidized-fuse`)
- **FSKit** - Apple's native FSKit framework (`oxidized-fskit`, macOS 15.4+)
...
```bash
cargo run -p oxidized-bench -- <vault_path> [options]
```
...
`oxbench` uses the unified `MountBackend` trait from `oxidized-cryptolib`:

```
oxidized-bench
      │
      ├── FuseBackend (from oxidized-fuse)
      ├── FSKitBackend (from oxidized-fskit)
...
Suite: full
Iterations: 10
Implementations: FUSE, FSKit

SEQUENTIAL READ (1MB)
╭───────────────┬────────────────┬────────────────┬──────────╮
│ Metric        │ FUSE           │ FSKit          │ Winner   │
├───────────────┼────────────────┼────────────────┼──────────┤
│ Throughput    │ 245.3 MB/s     │ 312.7 MB/s     │ ✓ FSKit  │
│ Mean Latency  │ 4.08 ms        │ 3.20 ms        │ ✓ FSKit  │
│ P50 Latency   │ 3.95 ms        │ 3.05 ms        │ ✓ FSKit  │
│ P95 Latency   │ 5.12 ms        │ 4.15 ms        │ ✓ FSKit  │
╰───────────────┴────────────────┴────────────────┴──────────╯
```
...
```bash
# Build
cargo build -p oxidized-bench

# Run tests
cargo test -p oxidized-bench

# Build release (faster benchmarks)
cargo build -p oxidized-bench --release
```
