...
fn test_create_directory_writes_dirid_backup() {
    ...
    // Find the encrypted directory path
    let storage_path = vault_ops
        .calculate_directory_storage_path(&DirId::root())
        .expect("Failed to get storage path");

    // Look for the .c9r directory containing dir.c9r with our dir_id
    let mut found_dirid = false;
    for entry in fs::read_dir(&storage_path).expect("Failed to read storage dir") {
        let entry = entry.expect("Failed to read entry");
        let path = entry.path();

        if path.is_dir() && path.extension().map_or(false, |e| e == "c9r") {
            ...
            if dir_c9r.exists() {
                ...
                if stored_id.trim() == dir_id.as_str() {
                    // Found our directory - check for dirid.c9r
                    let dirid_c9r = path.join("dirid.c9r");
                    assert!(
                        dirid_c9r.exists(),
                        "dirid.c9r should exist alongside dir.c9r"
                    );
                    found_dirid = true;
                    break;
                }
    ...
    assert!(found_dirid, "Should have found the directory with dirid.c9r backup");
}

#[test]
fn test_recover_parent_dir_id_basic() {
    use std::fs;
    ...
    // Find the child directory's encrypted path
    let parent_storage = vault_ops
        .calculate_directory_storage_path(&parent_id)
        .expect("Failed to get parent storage path");

    let mut child_encrypted_path = None;
    for entry in fs::read_dir(&parent_storage).expect("Failed to read parent storage") {
        let entry = entry.expect("Failed to read entry");
        let path = entry.path();
        ...
        if path.is_dir() && path.extension().map_or(false, |e| e == "c9r") {
            ...
            if dir_c9r.exists() {
                let stored_id = fs::read_to_string(&dir_c9r).expect("Failed to read dir.c9r");
                if stored_id.trim() == child_id.as_str() {
                    child_encrypted_path = Some(path);
                    break;
                }
            }
        }
    ...
    let child_path = child_encrypted_path.expect("Should find child directory");

    // Use recover_parent_dir_id to get the parent
    let recovered_parent = vault_ops
        .recover_parent_dir_id(&child_path, &child_id)
        .expect("Failed to recover parent dir id");

    assert_eq!(
        recovered_parent.as_str(),
        parent_id.as_str(),
        "Recovered parent ID should match original"
    );
}

#[test]
fn test_recover_parent_dir_id_root_parent() {
    use std::fs;
    ...
    let dir_path = encrypted_path.expect("Should find directory");

    // Recover should return root (empty string ID)
    let recovered_parent = vault_ops
        .recover_parent_dir_id(&dir_path, &dir_id)
        .expect("Failed to recover parent dir id");

    assert_eq!(
        recovered_parent.as_str(),
        DirId::root().as_str(),
        "Parent of root-level directory should be root"
    );
}
...
fn test_recover_directory_tree_basic() {
    ...
    let recovered = vault_ops
        ...
        .expect("Failed to recover directory tree");

    // Should have 3 directories
    assert_eq!(recovered.len(), 3, "Should recover 3 directories");

    // Find each directory in the recovered list
    let docs_info = recovered
        .iter()
        .find(|r| r.directory_id.as_str() == docs_id.as_str())
        .expect("Should find Documents");
    assert_eq!(
        docs_info.parent_directory_id.as_str(),
        DirId::root().as_str(),
        "Documents parent should be root"
    );

    let photos_info = recovered
        .iter()
        .find(|r| r.directory_id.as_str() == photos_id.as_str())
        .expect("Should find Photos");
    assert_eq!(
        photos_info.parent_directory_id.as_str(),
        DirId::root().as_str(),
        "Photos parent should be root"
    );

    let vacation_info = recovered
        .iter()
        .find(|r| r.directory_id.as_str() == vacation_id.as_str())
        .expect("Should find Vacation");
    assert_eq!(
        vacation_info.parent_directory_id.as_str(),
        photos_id.as_str(),
        "Vacation parent should be Photos"
    );
}
...
#[test]
fn test_recover_directory_tree_deeply_nested() {
    let (vault_path, master_key) = VaultBuilder::new().build();
    let vault_ops = VaultOperations::new(&vault_path, master_key);
    ...
    assert_eq!(recovered.len(), 5, "Should recover 5 directories");

    // Verify each level's parent
    let l1 = recovered.iter().find(|r| r.directory_id.as_str() == level1.as_str()).unwrap();
    assert_eq!(l1.parent_directory_id.as_str(), DirId::root().as_str());

    let l2 = recovered.iter().find(|r| r.directory_id.as_str() == level2.as_str()).unwrap();
    assert_eq!(l2.parent_directory_id.as_str(), level1.as_str());

    let l3 = recovered.iter().find(|r| r.directory_id.as_str() == level3.as_str()).unwrap();
    assert_eq!(l3.parent_directory_id.as_str(), level2.as_str());

    let l4 = recovered.iter().find(|r| r.directory_id.as_str() == level4.as_str()).unwrap();
    assert_eq!(l4.parent_directory_id.as_str(), level3.as_str());

    let l5 = recovered.iter().find(|r| r.directory_id.as_str() == level5.as_str()).unwrap();
    assert_eq!(l5.parent_directory_id.as_str(), level4.as_str());
}
...
fn test_dirid_backup_survives_rename() {
    ...
    // Find the renamed directory and verify dirid.c9r still exists
    let root_storage = vault_ops
        .calculate_directory_storage_path(&DirId::root())
        .expect("Failed to get root storage path");
    ...
    for entry in fs::read_dir(&root_storage).expect("Failed to read root storage") {
        ...
        if path.is_dir() {
            ...
            if dir_c9r.exists() {
                let stored_id = fs::read_to_string(&dir_c9r).expect("Failed to read dir.c9r");
                if stored_id.trim() == dir_id.as_str() {
                    // Found our directory - verify dirid.c9r exists
                    let dirid_c9r = path.join("dirid.c9r");
                    assert!(
                        dirid_c9r.exists(),
                        "dirid.c9r should still exist after rename"
                    );
                    found_dirid = true;

                    // Also verify the parent is still correctly recorded as root
                    let recovered_parent = vault_ops
                        .recover_parent_dir_id(&path, &dir_id)
                        .expect("Failed to recover parent");
                    assert_eq!(recovered_parent.as_str(), DirId::root().as_str());
                    break;
                }
            }
        }
    ...
    assert!(found_dirid, "Should find renamed directory with dirid.c9r");
}
...
fn test_shortened_directory_has_dirid_backup() {
    ...
    // Find the encrypted directory (should be .c9s)
    let root_storage = vault_ops
        .calculate_directory_storage_path(&DirId::root())
        .expect("Failed to get root storage path");
    ...
    for entry in fs::read_dir(&root_storage).expect("Failed to read root storage") {
        ...
        if path.is_dir() {
            ...
            if dir_c9r.exists() {
                let stored_id = fs::read_to_string(&dir_c9r).expect("Failed to read dir.c9r");
                if stored_id.trim() == dir_id.as_str() {
                    // Verify it's a .c9s directory
                    let path_str = path.to_string_lossy();
                    assert!(
                        path_str.ends_with(".c9s"),
                        "Long directory name should use .c9s format"
                    );

                    // Verify dirid.c9r exists
                    let dirid_c9r = path.join("dirid.c9r");
                    assert!(
                        dirid_c9r.exists(),
                        "dirid.c9r should exist in .c9s directory"
                    );

                    // Verify we can recover the parent
                    let recovered_parent = vault_ops
                        .recover_parent_dir_id(&path, &dir_id)
                        .expect("Failed to recover parent from .c9s directory");
                    assert_eq!(recovered_parent.as_str(), DirId::root().as_str());
    ...
    assert!(found_dirid, "Should find .c9s directory with dirid.c9r");
}
