...
use crate::{
    ...
    fs::file::{decrypt_file, encrypt_file_content, encrypt_file_header, DecryptedFile},
    fs::name::{
        create_c9s_filename, decrypt_filename, decrypt_parent_dir_id, encrypt_filename,
        encrypt_parent_dir_id, hash_dir_id, NameError,
    },
...
///
/// This struct is returned by `recover_directory_tree()` and contains
/// the recovered parent-child relationship for a directory.
#[derive(Debug, Clone)]
pub struct RecoveredDirectoryInfo {
    /// The directory's own ID
    pub directory_id: DirId,
    /// The recovered parent directory ID
    pub parent_directory_id: DirId,
    /// Path to the encrypted .c9r or .c9s directory
    pub encrypted_path: PathBuf,
}
...
impl VaultOperations {
    /// Create a new VaultOperations instance with default shortening threshold (220)
    ...
    pub fn new(vault_path: &Path, master_key: MasterKey) -> Self {
        Self::with_shortening_threshold(vault_path, master_key, DEFAULT_SHORTENING_THRESHOLD)
    }
    ...
    pub fn read_file(
        ...
        // Then decrypt it
        debug!(encrypted_path = %file_info.encrypted_path.display(), encrypted_size = file_info.encrypted_size, "Decrypting file");
    ...
    pub fn write_file(
        ...
        // 4. Generate a random content key
        let mut content_key = [0u8; 32];
        rand::rng().fill_bytes(&mut content_key);

        // 5. Encrypt the header
        let header = encrypt_file_header(&content_key, &self.master_key)?;

        // 6. Extract header nonce (first 12 bytes)
        ...
        // 8. Combine header + content
        let mut file_data = Vec::with_capacity(header.len() + encrypted_content.len());
        file_data.extend_from_slice(&header);
        file_data.extend_from_slice(&encrypted_content);

        // 9. Zeroize content key before any potential error returns
        content_key.zeroize();
    ...
    pub fn create_directory(
        ...
        if encrypted_name.len() > self.shortening_threshold {
            ...
            // Write dirid.c9r backup file (encrypted parent ID)
            let encrypted_parent_id =
                encrypt_parent_dir_id(parent_dir_id.as_str(), dir_id.as_str(), &self.master_key)?;
            self.atomic_write(&encrypted_dir_path.join("dirid.c9r"), &encrypted_parent_id)?;
        }
    ...
    pub fn rename_file(
        ...
        let file_data = if source_info.is_shortened {
    ...
    pub fn rename_directory(
        ...
        // Determine if source is shortened by checking for name.c9s
        let source_is_shortened = source_info.encrypted_path.join("name.c9s").exists();
        ...
        if new_is_long {
            ...
            let encrypted_parent_id =
                encrypt_parent_dir_id(parent_dir_id.as_str(), dir_id.as_str(), &self.master_key)?;
            self.atomic_write(&new_path.join("dirid.c9r"), &encrypted_parent_id)?;
        }

        // Remove the old directory entry (not the storage directory - that stays!)
        if source_is_shortened {
    ...
    fn create_shortened_directory(
        ...
        let encrypted_parent_id =
            encrypt_parent_dir_id(parent_dir_id.as_str(), dir_id.as_str(), &self.master_key)?;
        self.atomic_write(&short_dir.join("dirid.c9r"), &encrypted_parent_id)?;
    ...
    /// Recover a directory's parent relationship from its dirid.c9r backup file.
    ///
    /// This function reads the encrypted parent directory ID from the dirid.c9r
    /// file in a directory's .c9r folder and decrypts it using the directory's
    /// own ID as associated data.
    ///
    /// # Arguments
    ///
    /// * `encrypted_dir_path` - Path to the .c9r directory containing dirid.c9r
    /// * `dir_id` - The directory's own ID (from dir.c9r or known externally)
    ///
    /// # Returns
    ///
    /// The decrypted parent directory ID, or an error if recovery fails.
    ///
    /// # Errors
    ///
    /// - `VaultOperationError::Io`: If dirid.c9r cannot be read
    /// - `VaultOperationError::Filename`: If decryption fails (integrity violation)
    pub fn recover_parent_dir_id(
        &self,
        encrypted_dir_path: &Path,
        dir_id: &DirId,
    ) -> Result<DirId, VaultOperationError> {
        let dirid_file = encrypted_dir_path.join("dirid.c9r");
        let encrypted_parent_id = fs::read(&dirid_file)?;

        let parent_dir_id_str =
            decrypt_parent_dir_id(&encrypted_parent_id, dir_id.as_str(), &self.master_key)?;

        Ok(DirId::from_raw(parent_dir_id_str))
    }
